...
extension TwitterClient {
  ...
  func currentUser(completion: (user: User?) -> Void) {
    ...
    if (userId == nil) {
      ...
      sendRequest(.GET,endpoint: "users/show.json", parameters: ["user_id": userId!], completion: { (error, data, response) in
          if (error != nil) {
            print(error)
            completion(user: nil)
          } else {
...
extension TwitterClient {
  func sendRequest(method: OAuthSwiftHTTPRequest.Method, endpoint: String, parameters: [String: AnyObject]?, completion: (error: NSError?, data: NSData?, response: NSHTTPURLResponse?) -> Void) {
    ...
    oauth.client.request( url, method: method, parameters: parameters ?? [:], headers: nil, checkTokenExpiration: false, success: { (data, response) in
